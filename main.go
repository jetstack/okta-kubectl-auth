...
func cmd() *cobra.Command {
	...
	c := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			log.Info().Msgf("Please go to %s to login", authCodeURL)
			...
			case "https":
				log.Printf("listening on %s", listenURL)
				if err := a.server.ListenAndServeTLS(tlsCert, tlsKey); err != nil {
					return err
				}
			default:
